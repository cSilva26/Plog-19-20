...
% For now its very simple, but you can tweak it as you want.
...
% If start and destination is the same cell - its degenerated case, will make it always true.
...
% For capture using king we need to check that there are no figures on diagonal from current kings position (X1, Y2)
...
% Recursive predicates used to define a full capture (i.e. always continue capture, when its possible)
...
% Check is specified side can move. If it cant - it is considered as the end of the game.
